...
Kon = await getBuffer(`https://hardianto.xyz/api/welcome3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/642906f4e8b368fe2a020.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)

Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/642906f4e8b368fe2a020.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
...
Description: ${metadata.desc}
 ...
 نورتنا يا ماي فريند ممنوع الالفاظ والبرايفت واللينكات ⚠️ غير كدة كلنا اخوات واصحاب ❤️`} )
                } else if (anu.action == 'remove') {
                    XeonBotInc.sendMessage(anu.id, { image: Tol, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Left ${metadata.subject}

انت مش جدع حد يكون فى روم زى ده ويخرج 🙄 ده حتى كلنا اخوات واصحاب🥺 يلا بالسلامات ❤️😂` })
                }
            }
        } catch (err) {
            console.log(err)
